...
/**
 * Complete unit tests for {@link CourseController}.
 * This test class consolidates all Course-related controller tests.
 * 
 * @author LLLLLLLEEEEEEEEEEEEXXXXXXXXXXXXXXXXX
 */
...
class CourseControllerTest {
    ...
    class CourseCreationTests {
        ...
        void addCourse_WithValidData_ShouldReturnSuccess() throws Exception {
            // Given
            try (var mockedSecurityUtils = mockStatic(SecurityUtils.class)) {
                ...
                when(courseManager.addCourse(any(CourseCreationDto.class), eq(testUser))).thenReturn(testCourse);
    ...
    @Nested
    @DisplayName("Course Validation Tests")
    class CourseValidationTests {
        ...
        @Test
        @DisplayName("Should return bad request when course code is blank")
        void addCourse_WithBlankCode_ShouldReturnBadRequest() throws Exception {
            // Given
            CourseCreationDto invalidDto = new CourseCreationDto(
                    "Valid Name",
                    "", // blank code
        ...
        @Test
        @DisplayName("Should validate course creation DTO fields - excessive credits")
        void addCourse_WithExcessiveCredits_ShouldReturnBadRequest() throws Exception {
        ...
        @Test
        @DisplayName("Should validate course update DTO fields - negative credits")
        void updateCourse_WithNegativeCredits_ShouldReturnBadRequest() throws Exception {
        ...
        @Test
        @DisplayName("DTO JSON serialization and deserialization test")
        void testDtoSerialization() throws Exception {
            ...
            // Basic assertion to ensure no serialization errors
    ...
    class CourseSearchTests {
        ...
        @Test
        @DisplayName("Should return empty list when no courses match search keyword")
        void searchCourses_WithNoMatches_ShouldReturnEmptyList() throws Exception {
            // Given
            String keyword = "NonExistentCourse";
            when(courseManager.getCourses(keyword)).thenReturn(Collections.emptyList());
    ...
    @Nested
    @DisplayName("Course Filter Tests")
    class CourseFilterTests {
    ...
    class ErrorHandlingTests {
        ...
        @Test
        @DisplayName("Should handle service exceptions during course creation")
        void addCourse_WithServiceException_ShouldReturnInternalServerError() throws Exception {
            // Given
            try (var mockedSecurityUtils = mockStatic(SecurityUtils.class)) {
                mockedSecurityUtils.when(SecurityUtils::getCurrentUser).thenReturn(testUser);
                when(courseManager.addCourse(any(CourseCreationDto.class), eq(testUser)))
        ...
        @Test
        @DisplayName("Should handle service exceptions during course update")
        void updateCourse_WithServiceException_ShouldReturnInternalServerError() throws Exception {
            // Given
            try (var mockedSecurityUtils = mockStatic(SecurityUtils.class)) {
                mockedSecurityUtils.when(SecurityUtils::getCurrentUser).thenReturn(testUser);
                when(courseManager.updateCourse(any(CourseUpdateDto.class), eq(testUser)))
                ...
                mockMvc.perform(put("/course")
                        ...
                        .andExpect(status().isInternalServerError())
                        .andExpect(jsonPath("$.success").value(false))
                        .andExpect(jsonPath("$.message").value("更新课程失败"));
            }
        }
    }
}
